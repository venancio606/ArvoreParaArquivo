...
int main() {
    // 1. Criando a pasta principal (raiz) para servir de base
    Nodo *raiz = CriarNodo("raiz", 1, NULL);

    // 2. Criando alguns itens soltos para testar
    Nodo *arq1 = CriarNodo("z_arquivo.txt", 0, raiz);
    Nodo *arq2 = CriarNodo("a_arquivo.txt", 0, raiz);
    Nodo *pasta1 = CriarNodo("minha_pasta", 1, raiz);

    // 3. Colocando os itens dentro da pasta raiz usando sua função de ordenar
    OrdenarAntes(raiz->arquivo, arq1);
    OrdenarAntes(raiz->arquivo, arq2);
    OrdenarAntes(raiz->arquivo, pasta1);

    // 4. Testando o seu comando_ls!
    printf("Conteudo da pasta raiz (deve aparecer em ordem alfabetica):\n");
    printf("----------------------------------------------------------\n");
    comando_ls(raiz);
